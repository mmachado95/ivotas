...
public class TCPServer {
  ...
  private RMIServerInterface connectRMIInterface(String ip) {
    RMIServerInterface rmi = null;
    ...
    while (true) {
      try {
        // primeiro arg é o ip
        rmi = (RMIServerInterface) LocateRegistry.getRegistry(ip, this.getPort()).lookup("ivotas");
        //r.addAdmin(a);
...
class Connection extends Thread {
  ...
  public Connection(Socket aClientSocket, int number, String ip, TCPServer tableServer, ArrayList<String> votingTableMessages, CopyOnWriteArrayList<Connection> threads, RMIServerInterface rmi) {
  ...
  private RMIServerInterface connectRMIInterface(TCPServer server) {
    RMIServerInterface rmi = null;
    boolean noExceptions = false;

    while (true) {
      try {
        rmi = (RMIServerInterface) LocateRegistry.getRegistry(this.ip, server.getPort()).lookup("ivotas");
        //r.addAdmin(a);
        rmi.remote_print("New client");
        noExceptions = true;
      } catch (Exception e) {
...
class Menu extends Thread {
  ...
  public Menu (ArrayList<String> votingTableMenuMessages, CopyOnWriteArrayList<Connection> votingTerminals, RMIServerInterface rmi, TCPServer tableServer, int votingTableId, String ip) {
